...
for X3D in WalkingAlienX_ITE_Final.x3d conan_23_Aug2025_Final.x3d walking_man_cc_test_Final.x3d JoeHAnimKick1a_Final.x3d JoeDemo5JoeSkin5_Final.x3d 
do
	X3DV="${X3D}v"
	X3DOM=`basename "${X3D}" Final.x3d`x3dom.x3d
	echo "Converting Final Output from .x3d to .x3dv..."
	npx x3d-tidy@latest -i "${X3D}" -o "${X3DV}"
	if [ "${X3DV}" = "WalkingAlienX_ITE_Final.x3dv" ]
	then
	...
	echo "Validating..."
	~/Downloads/castle-model-viewer-5.3.0-win64-x86_64/castle-model-viewer/castle-model-converter.exe --validate "${X3DV}"

	echo "Commenting out excess ROUTES (no joints present)..."
	(~/Downloads/castle-model-viewer-5.3.0-win64-x86_64/castle-model-viewer/castle-model-converter.exe --validate "${X3DV}" 2>&1 | grep "not found"|sed 's/" not found//'| sed 's/.*Route destination node name "//'| sort -u | sed 's/\(.*\)/%s\/^\\(.*ROUTE.*\1.*\\)\/# \\1\//'; echo wq) > "${X3DV}".cmds
	ex "${X3DV}" < "${X3DV}".cmds
