...
def main(input_file: str = None, output_file: str = None):
    ...
    if root_joint is not None:
        ...
        try:
            scaleHeight = center[1] / HAnimJointNames.HUMANOID_ROOT_HEIGHT
            scale = (scaleHeight, scaleHeight, scaleHeight)
            print(f"scale is {scale}")
            transform_node(root, scale, tree)
...
def transform_point(point: List[float], point_offset: int, scale: List[float]) -> List[float]:
...
def transform_position_interpolator(node: ET.Element, scale: List[float]) -> bool:
    ...
    for i in range(0, len(key_values), 3):
        if i + 2 < len(key_values):
            key_values = transform_point(key_values, i, scale)
...
def transform_node(node: ET.Element, scale: List[float], parent_node: ET.Element) -> bool:
    ...
    if node_tag.endswith('HAnimHumanoid') or node_tag == 'HAnimHumanoid':
        ...
        return transform_position_interpolator(node, scale)
    ...
    for child in children:
        if child is not None:
            if not transform_node(child, scale, node):
                print(f"Unpacking child {child.tag} failed")
